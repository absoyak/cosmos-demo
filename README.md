...
orbiting a star that was born from the collisions. All physics runs on the GPU (WebGL2).

No build step, no dependencies: open `index.html` in a current Chrome, Edge or Firefox.
...
It can also be set from the URL (`index.html?cosmos=40&seed=7`) or from the box in the top-right corner.
Every universe is generated from a seed, shown in the HUD, so a good one can be replayed.
...
Small bodies are lumpy rocks; they round out as they gain mass. Zoom in and planets have surfaces
(rocky, gas giant, ocean, ice), are lit by the star, and spin. A body that gathers 1 % of all mass ignites as a star.
...
full-screen fragment shader passes:
...
- **Rendering** uses no vertex buffers at all — quads and lines are generated from `gl_VertexID` /
  `gl_InstanceID`, and planets are shaded as sphere impostors.
...
URL for experiments, e.g. `?GAS_DRAG=0&CORE_MASS=0.9`.
